...
// ----------- HELPERS -----------
const toNumber = (v, f = 0) => (isNaN(Number(v)) ? f : Number(v));

const deriveOrderType = (order) => {
  if (order?.tableNo && order.tableNo !== "NA") return "Table/Room";
  if (/^\d{10}$/.test(order?.customerMobile)) return "Delivery";
  return "Takeaway";
};
...
const sanitizeOrderForUi = (order) => {
  ...
  return {
    ...
    type: deriveOrderType(order),
    itemsList,
    total: order.total || itemsList.reduce((s, i) => s + i.qty * i.price, 0),
  };
};
...
export default function Orders() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setOrders(data.map(sanitizeOrderForUi));
    setLoading(false);
  }, []);
  ...
  // -------- FILTER --------
  const filteredOrders = useMemo(() => {
    return orders.filter((o) => o.type === orderType && o.status === status);
  }, [orders, orderType, status]);

  // -------- REVENUE --------
  const totalRevenue = orders.reduce((sum, o) => {
    return o.status === "Order Completed" ? sum + o.total : sum;
  }, 0);
  ...
  const updateStatus = async (id, uiStatus) => {
    ...
    await fetch("/api/orders", {
      ...
      body: JSON.stringify({
        ...
        status: apiStatus,
        acceptedAt:
          apiStatus === "Preparing" ? new Date().toISOString() : undefined,
        preparingAt:
          apiStatus === "Preparing" ? new Date().toISOString() : undefined,
        completedAt:
          apiStatus === "Completed" ? new Date().toISOString() : undefined,
        cancelledAt:
          apiStatus === "Cancelled" ? new Date().toISOString() : undefined,
      }),
    });
      ...
      {loading ? (
        ...
        filteredOrders.map((order) => (
          <div key={order.id} className="border p-4 mb-4">
            ...
            {/* Toggle Items */}
            <button onClick={() => setOpenOrder(order.id)}>View Items</button>

            {openOrder === order.id && (
              <div>
                {order.itemsList.map((i) => (
                  <p key={i.id}>
                    {i.name} x {i.qty}
                  </p>
                ))}
            ...
            {order.status === "Preparing" && (
              <button onClick={() => updateStatus(order.id, "Order Completed")}>
                Complete
              </button>
            )}
