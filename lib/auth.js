...
export const normalizeUserRole = (value) => {
  const normalized = String(value || "")
    .trim()
    .toLowerCase();
  return USER_ROLES.includes(normalized) ? normalized : "customer";
};

export const shouldGrantAdminRole = ({
  username,
  email,
  ...
  const configuredAdminEmails = new Set(parseEnvList(process.env.ADMIN_EMAILS));

  if (
    (normalizedUsername && configuredAdminUsernames.has(normalizedUsername)) ||
    (normalizedEmail && configuredAdminEmails.has(normalizedEmail))
  ) {
    return true;
  }

  return Number(existingAdminCount) <= 0;
};
