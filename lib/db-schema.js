...
const ensureCoreTablesInternal = async () => {
  ...
  await pool.query(`
    ...
    WHERE role IS NULL
      OR TRIM(role) = ''
      OR LOWER(TRIM(role)) NOT IN ('admin', 'customer');
  `);
  ...
  await pool.query(`
    ...
    UPDATE app_users
    SET role = 'admin'
    WHERE id IN (SELECT id FROM oldest_user)
      AND NOT EXISTS (
        SELECT 1
        FROM app_users
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS inventory_movements (
      ...
      menu_item_name VARCHAR(120) NOT NULL DEFAULT '',
      quantity_change INTEGER NOT NULL DEFAULT 0,
      stock_before INTEGER NOT NULL DEFAULT 0,
      stock_after INTEGER NOT NULL DEFAULT 0,
      movement_type VARCHAR(20) NOT NULL DEFAULT 'deduct',
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
  `);
  ...
  await pool.query(`
    ...
    ADD COLUMN IF NOT EXISTS inventory_item_id TEXT DEFAULT '',
    ADD COLUMN IF NOT EXISTS menu_item_name VARCHAR(120) DEFAULT '',
    ...
    ADD COLUMN IF NOT EXISTS movement_type VARCHAR(20) DEFAULT 'deduct',
    ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
  `);
  ...
  await pool.query(`
    ...
    ADD COLUMN IF NOT EXISTS item_name VARCHAR(120) DEFAULT '',
  ...
  await pool.query(`
    CREATE INDEX IF NOT EXISTS low_stock_alerts_active_idx
    ON low_stock_alerts (is_active, updated_at DESC);
  `);
