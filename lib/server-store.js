import { promises as fs } from "fs";
import os from "os";
import path from "path";
import { createDefaultReservationPayload } from "@/lib/reservation-settings-defaults";

const STORE_DIR = path.join(process.cwd(), "data");
const STORE_FILE = path.join(STORE_DIR, "restaurant-store.json");
const TMP_STORE_FILE = path.join(
  os.tmpdir(),
  "swadpoint",
  "restaurant-store.json"
);

const DEFAULT_STORE = {
  users: [],
  menu: [],
  inventory: [],
  orders: [],
  payments: [],
  reservationSettings: createDefaultReservationPayload(),
};

let writeQueue = Promise.resolve();
let resolvedStoreFilePromise = null;

const normalizeStore = (raw) => ({
  users: Array.isArray(raw?.users) ? raw.users : [],
  menu: Array.isArray(raw?.menu) ? raw.menu : [],
  inventory: Array.isArray(raw?.inventory) ? raw.inventory : [],
  orders: Array.isArray(raw?.orders) ? raw.orders : [],
  payments: Array.isArray(raw?.payments) ? raw.payments : [],
  reservationSettings:
    raw?.reservationSettings && typeof raw.reservationSettings === "object"
      ? raw.reservationSettings
      : createDefaultReservationPayload(),
});

const ensureStoreFile = async (filePath) => {
  const dir = path.dirname(filePath);
  await fs.mkdir(dir, { recursive: true });

  try {
    await fs.access(filePath);
  } catch {
    await fs.writeFile(filePath, JSON.stringify(DEFAULT_STORE, null, 2), "utf8");
  }
};

const getStoreFilePath = async () => {
  if (!resolvedStoreFilePromise) {
    resolvedStoreFilePromise = (async () => {
      const candidates = [STORE_FILE, TMP_STORE_FILE];

      for (const filePath of candidates) {
        try {
          await ensureStoreFile(filePath);
          return filePath;
        } catch {
          // Try next path.
        }
      }

      throw new Error("No writable store path available");
    })();
  }

  return resolvedStoreFilePromise;
};

const readStore = async () => {
  const filePath = await getStoreFilePath();

  try {
    const raw = await fs.readFile(filePath, "utf8");
    return normalizeStore(JSON.parse(raw));
  } catch {
    return { ...DEFAULT_STORE };
  }
};

const writeStore = async (store) => {
  const filePath = await getStoreFilePath();
  const nextStore = normalizeStore(store);
  await fs.writeFile(filePath, JSON.stringify(nextStore, null, 2), "utf8");
  return nextStore;
};

const withWriteLock = async (task) => {
  const next = writeQueue.then(task, task);
  writeQueue = next.catch(() => {});
  return next;
};

export const getOrdersFromStore = async () => {
  const store = await readStore();
  return store.orders;
};

export const getUsersFromStore = async () => {
  const store = await readStore();
  return store.users;
};

export const findUserByIdentifierInStore = async (identifier) => {
  const normalizedIdentifier = String(identifier || "").trim().toLowerCase();
  if (!normalizedIdentifier) return null;

  const store = await readStore();
  return (
    store.users.find((user) => {
      const username = String(user?.username || "").trim().toLowerCase();
      const email = String(user?.email || "").trim().toLowerCase();
      return (
        normalizedIdentifier === username || normalizedIdentifier === email
      );
    }) || null
  );
};

export const createUserInStore = async (user) =>
  withWriteLock(async () => {
    const store = await readStore();
    const username = String(user?.username || "")
      .trim()
      .toLowerCase();
    const email = String(user?.email || "")
      .trim()
      .toLowerCase();

    const duplicate = store.users.find((storedUser) => {
      const existingUsername = String(storedUser?.username || "")
        .trim()
        .toLowerCase();
      const existingEmail = String(storedUser?.email || "")
        .trim()
        .toLowerCase();
      return (
        (username && existingUsername === username) ||
        (email && existingEmail === email)
      );
    });

    if (duplicate) return null;

    store.users.push(user);
    await writeStore(store);
    return user;
  });

export const getMenuFromStore = async () => {
  const store = await readStore();
  return store.menu;
};

export const replaceMenuInStore = async (menuItems) =>
  withWriteLock(async () => {
    const store = await readStore();
    store.menu = Array.isArray(menuItems) ? menuItems : [];
    await writeStore(store);
    return store.menu;
  });

export const getInventoryFromStore = async () => {
  const store = await readStore();
  return store.inventory;
};

export const replaceInventoryInStore = async (inventoryItems) =>
  withWriteLock(async () => {
    const store = await readStore();
    store.inventory = Array.isArray(inventoryItems) ? inventoryItems : [];
    await writeStore(store);
    return store.inventory;
  });

export const addInventoryItemToStore = async (item) =>
  withWriteLock(async () => {
    const store = await readStore();
    store.inventory.push(item);
    await writeStore(store);
    return item;
  });

export const updateInventoryItemInStore = async (id, partial) =>
  withWriteLock(async () => {
    const store = await readStore();
    const index = store.inventory.findIndex((item) => item?.id === id);
    if (index === -1) return null;

    const updatedItem = {
      ...store.inventory[index],
      ...partial,
    };
    store.inventory[index] = updatedItem;
    await writeStore(store);
    return updatedItem;
  });

export const deleteInventoryItemFromStore = async (id) =>
  withWriteLock(async () => {
    const store = await readStore();
    const beforeLength = store.inventory.length;
    store.inventory = store.inventory.filter((item) => item?.id !== id);
    if (store.inventory.length === beforeLength) return false;
    await writeStore(store);
    return true;
  });

export const addOrderToStore = async (order) =>
  withWriteLock(async () => {
    const store = await readStore();
    store.orders.push(order);
    await writeStore(store);
    return order;
  });

export const updateOrderInStore = async (id, partial) =>
  withWriteLock(async () => {
    const store = await readStore();
    const index = store.orders.findIndex((order) => order?.id === id);
    if (index === -1) return null;

    const updatedOrder = {
      ...store.orders[index],
      ...partial,
    };
    store.orders[index] = updatedOrder;
    await writeStore(store);
    return updatedOrder;
  });

export const getPaymentsFromStore = async () => {
  const store = await readStore();
  return store.payments;
};

export const addPaymentToStore = async (payment) =>
  withWriteLock(async () => {
    const store = await readStore();
    store.payments.push(payment);
    await writeStore(store);
    return payment;
  });

export const getReservationSettingsFromStore = async () => {
  const store = await readStore();
  return store.reservationSettings || createDefaultReservationPayload();
};

export const replaceReservationSettingsInStore = async (payload) =>
  withWriteLock(async () => {
    const store = await readStore();
    store.reservationSettings =
      payload && typeof payload === "object"
        ? payload
        : createDefaultReservationPayload();
    await writeStore(store);
    return store.reservationSettings;
  });
